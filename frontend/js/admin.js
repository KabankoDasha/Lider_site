document.addEventListener('DOMContentLoaded', () => {
    ...
    saveApplicationBtn.addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/applications/admin/${currentAppId}`, {
                method: 'PUT',
                headers: {
    ...
    approveReviewBtn.addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/reviews/admin/${currentReviewId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    document.getElementById('confirm-delete-course').addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/courses/${currentCourseToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!response.ok) throw new Error('Ошибка удаления');
    ...
    document.getElementById('confirm-delete-sale').addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/sales/${currentSaleToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!response.ok) throw new Error('Ошибка удаления');
    ...
    document.getElementById('confirm-delete-instructor').addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/instructors/${currentInstructorToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!response.ok) throw new Error('Ошибка удаления');
    ...
    document.getElementById('save-new-instructor').addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (editingInstructorId) {
                response = await fetch(`/api/instructors/${editingInstructorId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                    body: JSON.stringify(data)
                });
    ...
    async function loadAgreements() {
        ...
        try {
            ...
            const docsRes = await fetch('/api/documents/bulk-check', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        container.querySelectorAll('.btn--preview-pdf').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                const token = localStorage.getItem('token');
                try {
                    const pdfRes = await fetch(`/api/agreements/${agreementId}/pdf`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
    ...
    document.getElementById('send-reply').addEventListener('click', async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`/api/applications/${currentReplyApplicationId}/reply`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
